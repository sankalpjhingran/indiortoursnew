...
angular.module('clientApp')
    .controller('LocationAdminController', function($scope, $uibModal, $http, $location, $document, $log, Upload, $timeout) {
        ...
        $scope.uploadFiles = function(tempLocation) {
          const files = $scope.locationData.images
          const fd = new FormData();
          for (let i = 0; i < files.length; i++) {
                fd.append('file', files[i]);
            }
            fd.append('parentobjectid', tempLocation.id);
            fd.append('parentobjectname', 'location');
        ...
        $scope.deleteUploadedFile = function(idx) {
          ...
          const file = $scope.locationData.newImages[idx];
          console.log(file);
          if (file && file.id) {
        ...
        $scope.createUpdateLocation = function() {
            ...
            if ($scope.locationData && $scope.locationData.id && $scope.locationData.country && $scope.locationData.region && $scope.locationData.continent) {
                ...
                $http.patch('/api/location/update/', $scope.locationData).then(function(res, err) {
                    if (res.status === 200) {
                        //Upload Images
                        if($scope.locationData.images) {
